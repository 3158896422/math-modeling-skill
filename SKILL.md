...
3. 严格调用 Skill 提供的脚本和模板。已有初始化、转换、编译或校验工具时，禁止为了省时手写替代实现。
   建模分析还必须先按 [通用建模证据与验证契约](references/通用建模证据与验证.md) 建立每个子问题的模型合同；模型合同、基线对照、信息时序和分层验证是后续门禁的输入，不是论文阶段的补写项。
...
- `M1` 必须检查模型合同和合理基线；`P1` 必须检查公式/约束的真实实现与小实例或边界证据；`P2` 必须检查与题型匹配的分层验证、情景/灵敏度和随机稳定性；`W1/W2` 必须检查每条主张的证据强度、信息时序、适用边界和决策表。具体条款见 [通用建模证据与验证契约](references/通用建模证据与验证.md)。
...
目标为 CUMCM 2026 时，必须使用用户提供的当届官方模板；内置模板只能作为技术基线，不能冒充官方模板。纸质版固定页面顺序为第 1 页承诺书、第 2 页编号专用页、第 3 页摘要专用页、第 4 页起正文，不要目录；摘要原则上不超过 1 页；本 Skill 默认要求完整渲染 PDF 总页数不超过 30 页，正文页数另作诊断统计；摘要页起页脚用阿拉伯数字从 1 连续编号。渲染完整 PDF 后必须先运行 `tools/docx/scripts/cumcm_submission.py validate-paper --mode paper`，再用 `export-electronic` 删除前两页并保证电子版首页为摘要、大小不超过 20 MB；支撑材料用 ZIP（或外部工具生成 RAR）打包为一个文件，不超过 20 MB，且包含文件清单。完整编程代码不粘贴在论文末尾，放入支撑材料 ZIP；论文附录只保留支撑材料文件清单、复现说明和必要中间结果。论文正文、附录及支撑材料中的姓名、学校、赛区、参赛编号等身份信息必须用通用敏感词和用户提供的真实词条扫描，自动扫描通过后仍需人工复核。

## Markdown 数学公式分隔符

为了兼容 VS Code Markdown 预览以及常见的 Markdown 数学渲染器，所有 Markdown 产物
（包括 `题目分析报告.md`、`术语表格.md`、阶段记录和论文草稿）统一使用以下分隔符：

- 行内公式使用 `$...$`，例如 `$x_i \in [0,1]$`。
- 独立公式使用成对且各自独占一行的 `$$...$$`，例如：
...
- Markdown 中禁止使用 `\(...\)` 或 `\[...\]` 作为公式分隔符，也不要混用多套分隔符。
- 该约束只适用于 Markdown 文本；复制到 `.tex` 文件的 LaTeX 源码仍按目标竞赛模板和 LaTeX 语法编写。
- 提交 Markdown 产物前，检查并改写遗留的 `\(...\)`、`\[...\]` 分隔符；代码块、LaTeX 源码示例和正则表达式中的字面量除外。
...
其中 `题目分析报告.md` 必须包含每个子问题的模型合同、基线/简化对照计划、信息可得性和验证计划；不另设固定的“模型合同”交付文件。
...
适用时，结果表还应保留基线对照、验证指标、情景/灵敏度和多初值/多种子汇总；单次最优结果不能作为稳定性证据。
...
| 通用模型合同、基线、验证和主张强度 | `references/通用建模证据与验证.md` |
...
- 所有计算结论来自实际运行结果。
- 公式、表格和图表与代码结果一致。
- 每个子问题都能沿“题目机制/信息时点 → 模型 → 算法 → 验证 → 决策结论 → 适用边界”复核；合理基线、正确性证据、题型匹配的分层验证和关键情景分析已完成，或已记录不适用理由与替代证据。
