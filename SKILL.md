...
目标为 CUMCM 2026 时，必须使用用户提供的当届官方模板；内置模板只能作为技术基线，不能冒充官方模板。纸质版固定页面顺序为第 1 页承诺书、第 2 页编号专用页、第 3 页摘要专用页、第 4 页起正文，不要目录；摘要原则上不超过 1 页，正文不超过 30 页，摘要页起页脚用阿拉伯数字从 1 连续编号。渲染完整 PDF 后必须先运行 `tools/docx/scripts/cumcm_submission.py validate-paper --mode paper`，再用 `export-electronic` 删除前两页并保证电子版首页为摘要、大小不超过 20 MB；支撑材料用 ZIP（或外部工具生成 RAR）打包为一个文件，不超过 20 MB，且包含文件清单。论文正文、附录及支撑材料中的姓名、学校、赛区、参赛编号等身份信息必须用通用敏感词和用户提供的真实词条扫描，自动扫描通过后仍需人工复核。
