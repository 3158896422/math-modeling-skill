...
6. 交付前运行当前角色规定的全部完成门禁。任一命令未运行、退出码非零、仍有未处理问题或产物在门禁后发生变化时，不得声称“已完成”。
7. 最终回复列出实际读取的入口、实际运行的关键命令、退出码、核心质量指标和仍存在的阻塞；不得只说“已检查”。
...
- Word：`完整论文.docx`。
- LaTeX：`完整论文-LaTeX/` 源码项目和由其实际编译得到的 `完整论文.pdf`。
...
- 论文已按目标竞赛当届官方规则配置，篇幅目标已经确认，公式/图/表数量经过检查，图表编号与正文引用连续，参考文献与正文引用双向对应。Word 分支还须通过原生 OMML、DOCX 结构和渲染页数检查；LaTeX 分支还须通过真实编译、日志、源码包和 PDF 实际页数检查。
