...
- 🧾 **证据链建模**：为每个子问题建立模型合同，要求合理基线、信息时序审计、算法可行性证据、分层验证和适用边界，按题型条件化执行。
...
|:---:|---|---|---|---|
| ① | [建模手](references/roles/建模手/SKILL.md) | 理解题目、建立模型合同、选择基线、定义算法和验证方案 | `M1` 建模终检 | `题目分析报告.md`、`术语表格.md` |
...
质检 Subagent 是阶段内只读验收者，不是第四个固定角色。默认只启用固定质检；其他协作仅在用户明确选择后运行。`P1` 在全量计算和正式出图前执行，`W1` 在长篇正文和双格式排版前执行；禁止等全流程结束后才首次质检。五个门禁的通用证据标准见 [通用建模证据与验证契约](references/通用建模证据与验证.md)，完整调度协议见 [Subagent 调度与阶段门禁](references/Subagent调度.md)。
...
- CUMCM 默认以约 15000 字词单位、约 20 页、5 个公式、8 幅图、3 个表作为可按题目调整的质量目标（非官方最低数量）；每个子问题至少一幅正式结果图。
- **CUMCM 提交合规**：全国赛必须使用当届官方模板；自动检查纸质版承诺书→编号专用页→摘要页→正文顺序、无目录、摘要原则上不超过 1 页和匿名信息；本 Skill 额外要求完整渲染 PDF 总页数不超过 30 页（正文页数仍按当届官方规则单独核对）。电子版删除前两页后首页为摘要，单文件建议 PDF 且不超过 20 MB；完整编程代码不粘贴到论文末尾，放入不超过 20 MB 的支撑材料 ZIP，论文附录只保留文件清单、复现说明和必要中间结果。

完整流程与门禁见 `tools/docx/SKILL.md`、`tools/latex/SKILL.md` 和 `tools/docx/scripts/cumcm_submission.py`。
...
回归测试覆盖双引擎搜索、公式转换、DOCX、LaTeX 模板与校验、Excel 重算、论文结构、动态依赖、复现清单和科学绘图工具；模型合同与证据门禁的关键路由也纳入回归检查。

## 📋 版本与更新日志

当前版本：[`1.3.0`](VERSION)

`1.3.0` 增加跨题型的模型合同、基线对照、信息时序审计、分层验证、情景稳健性和主张强度门禁。详细内容见 [CHANGELOG.md](CHANGELOG.md)。
