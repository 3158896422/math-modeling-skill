...
CUMCM 的约 15000 字词单位、约 20 页、5 个公式、8 幅图和 3 个表只是可按题目调整的完整度质量目标，不是官方最低数量；低于目标应输出质量预警，不得冒充官方规则违规。CUMCM 2026 的硬约束是摘要原则上不超过 1 页、正文不超过 30 页；电子版 PDF 首页必须是摘要、不得包含承诺书和编号专用页，且不超过 20 MB。页数上限等官方硬约束必须从目标届次规则读取后通过参数传入。MCM/ICM 不内置永久页数阈值。
