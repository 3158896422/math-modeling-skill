...
论文包含附录时再加 `--appendix-start-page <附录在 PDF 中的第一页>`，且源码必须真实包含 `\appendix` 或 `appendices` 环境。CUMCM 电子版 PDF 若第 1 页为摘要、正文从第 2 页开始，可传 `--body-start-page 2`；必须以实际 PDF 和当届官方模板为准。工具会尝试根据附录首个章节标题自动定位，但无法唯一定位时会阻断并要求显式页码。`--max-pages` 对 CUMCM 检查完整渲染 PDF 总页数，不用它猜测正文页数；`mcm-icm` 与 `generic` 的 `--max-pages` 默认校验 PDF 总页数，也不接受正文/附录页码参数。
...
CUMCM 的约 15000 字词单位、约 20 页、5 个公式、8 幅图和 3 个表只是可按题目调整的完整度质量目标，不是官方最低数量；低于目标应输出质量预警，不得冒充官方规则违规。摘要原则上不超过 1 页，官方正文页数口径必须按当届规则核对；本 Skill 默认要求完整渲染 PDF 总页数不超过 30 页，正文页数另作诊断统计。电子版 PDF 首页必须是摘要、不得包含承诺书和编号专用页，且不超过 20 MB。CUMCM 的 `--max-pages` 默认按本 Skill 的完整 PDF 总页数门槛 30 检查；当届官方规则或用户指定不同值时，显式传入新值并在报告中记录依据。MCM/ICM 不内置永久页数阈值。
...
CUMCM 输出时，完整编程代码不粘贴在论文末尾，也不在论文附录中逐文件展开。将可运行源程序、运行环境/依赖、运行命令、输入输出清单和必要中间结果统一放入一个不超过 20 MB 的支撑材料 ZIP（需要 RAR 时可用外部工具生成）；论文附录只保留支撑材料文件列表、复现说明和必要中间结果。该归档方式是本 Skill/用户指定的输出策略，可能与部分官方规范对附录源代码的要求不同，正式参赛前必须人工核对当届官方规则。
