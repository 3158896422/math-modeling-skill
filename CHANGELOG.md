...
- 新增 `references/通用建模证据与验证.md`，将历届优秀论文共同体现的“机制—模型—算法—验证—决策—边界”证据链转化为可执行契约。
- 建模手在 `M1` 前为每个子问题建立模型合同，明确对象、信息时点、变量单位、假设、目标约束、参数来源、代码映射、验证和失败条件。
- `M1` 增加合理基线/简化或精确对照要求；`P1` 增加公式/约束实现、小实例、边界/守恒和训练评估隔离等正确性证据；`P2` 增加按题型选择的分层验证、情景/灵敏度和多初值/多种子稳定性证据。
- `W1/W2` 增加主张强度、信息时序、决策表、近似并列结果和适用边界检查；正式图按主张—证据关系选取，数量目标不能替代证据。
- 更新三个角色的作者自检清单、阶段调度协议和 dsh Workbench 门禁提示，并加入证据契约路由回归测试。
- 以上要求按题型条件化执行；不适用的验证必须写明理由和替代证据，不强制套用固定模型或统计检验。

### CUMCM 2026 提交合规与质量目标语义

- **本次修订**：CUMCM 论文门禁改为检查完整渲染 PDF 总页数，默认不超过 30 页；正文页数保留为诊断指标。
- **代码归档策略**：论文末尾不再粘贴完整编程代码；完整可运行代码、依赖、运行命令和必要中间结果统一放入支撑材料 ZIP，论文附录只保留清单与复现说明。该策略是本 Skill/用户指定的输出门槛，与部分官方规范对附录源代码的要求可能不同，正式参赛前必须人工核对当届规则。

- 新增提交合规工具 `tools/docx/scripts/cumcm_submission.py`：检查纸质版页面顺序/正文边界、电子版删除承诺书与编号页、PDF 与支撑材料 20MB 上限、支撑材料 ZIP 文件清单，以及通用加自定义敏感词的匿名扫描。
- 将 CUMCM 约 15000 字词单位、约 20 页、5 公式、8 图、3 表明确为可按题目调整的质量目标；低于目标只产生质量预警，不冒充官方违规。
- 明确 2026 全国赛硬约束：摘要原则上不超过 1 页；本 Skill 默认要求完整渲染 PDF 总页数不超过 30 页，正文页数另作诊断统计；全国赛必须使用当届官方模板。同步更新论文流程、格式规范、自审清单和 dsh-plugin 知识库。

### Markdown 数学公式分隔符统一

- Markdown 产物统一使用 `$...$` 行内公式和单独成行的 `$$...$$` 行间公式，兼容 VS Code 预览。
- 明确禁止在 Markdown 中使用 `\(...\)`、`\[...\]` 作为公式分隔符，同时保留 `.tex` 源码的原生 LaTeX 语法。
- 同步更新建模手、论文手规范、dsh-plugin 内置知识库和回归测试。
...
- 将 CUMCM 的约 15000 字词单位、约 20 页明确为可覆盖的完整度质量目标；依据 2026 年官方规范，将正文页数按当届官方规则核对；本 Skill 默认要求完整渲染 PDF 总页数不超过 30 页作为当前已核验的硬约束，禁止混淆二者。
