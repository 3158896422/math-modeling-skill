...
当届官方规则要求纸质版第 1 页为承诺书、第 2 页为编号专用页、第 3 页为摘要专用页、第 4 页起为正文，不要目录；摘要原则上不超过 1 页，官方正文页数口径必须按当届文件核对。本 Skill 默认额外设置完整渲染 PDF 总页数不超过 30 页的输出门槛，正文页数另作诊断统计；摘要页起页脚用阿拉伯数字从 1 连续编号，附录在正文之后。电子版只保留一个 PDF 或 Word 文件且建议 PDF：必须删除承诺书和编号专用页，使摘要成为第 1 页，大小不超过 20 MB。支撑材料压缩为一个 RAR/ZIP，不超过 20 MB，并包含文件清单；所有论文正文、附录和支撑材料都不得出现姓名、学校、赛区、参赛编号等身份信息。
