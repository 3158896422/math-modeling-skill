...
### CUMCM 2026 官方约束与质量目标

- 官方硬约束（须按当届公告核对）：摘要原则上不超过 1 页；纸质版第 1 页承诺书、第 2 页编号专用页、第 3 页摘要专用页、第 4 页起正文；不要目录；正文页数按官方口径核对；正文后附录；摘要页起连续编页。电子版首页必须是摘要专用页，删除承诺书和编号专用页；建议单个 PDF 且不超过 20 MB。支撑材料压缩为一个 RAR/ZIP，不超过 20 MB，并在论文附录列清单。摘要、正文、附录和支撑材料全部匿名。
- 本 Skill 输出门槛与质量目标：完整渲染 PDF 总页数不超过 30 页；默认约 15000 字词单位、约 20 页、5 个公式、8 幅图、3 个表。公式、图和表可根据题目增加或减少，数量不足只产生质量预警；完整代码不粘贴到论文末尾，统一放入支撑材料 ZIP，附录只列清单、复现说明和必要中间结果。
- 页数、页面顺序和文件大小必须以 DOCX/LaTeX 渲染后的实际 PDF 为准；自动文本定位后仍需人工抽检。
- 全国赛生成必须使用当届官方模板；内置模板只是构建基线，缺少官方模板时报告阻塞。
...
该检查会把篇幅、公式/图/表数量不足标记为质量目标预警；图表编号与正文引用、参考文献双向对应仍属于完整性检查。默认约 15000 字词单位、20 页、公式 5 个、图 8 幅、表 3 个均为 CUMCM 质量目标；其他竞赛同样默认至少 8 幅正式图。2026 年 CUMCM 摘要 1 页原则和正文 30 页上限属于已经核验的官方硬约束。渲染完整 PDF 后继续运行 `cumcm_submission.py` 检查页面顺序、电子版删页、20 MB 和匿名信息。OOXML 校验通过不等于版面正确，仍需渲染抽检。
