...
同时记录两类约束：一类是当届官方页面顺序、摘要页、正文页数、提交文件、支撑材料和匿名限制，必须满足；另一类是论文完整度质量目标，可以按题目或用户要求调整。CUMCM 未给出更具体要求时，可按约 15000 字词单位、约 20 页、5 公式、8 图、3 表规划，但这些都是质量目标，不是官方最低数量。2026 年官方规范要求摘要原则上不超过 1 页、正文不超过 30 页，电子版删除承诺书和编号专用页，支撑材料和提交文件均不超过 20 MB。
...
1. 全国赛从当届官方参考模板创建文档；模板正文示例必须清除，但保留样式、分节、页眉页脚和页面设置。没有官方模板时只能报告阻塞，不能用内置模板冒充官方提交模板。纸质版固定页面顺序为承诺书、编号专用页、摘要页和正文；不要插入目录。
...
最后一条仅适用于 LaTeX/Markdown 经 Pandoc 转换得到的 DOCX。另需调用 `paper_format.validate_paper_structure()` 检查篇幅、公式/图/表数量、图表正文引用与参考文献对应关系，并把渲染后的实际页数传入 `rendered_pages`；数量不足是质量预警，结构和引用缺口是错误。随后渲染完整 PDF，运行 `cumcm_submission.py` 分别核验纸质版页面顺序、电子版删页、20 MB 与匿名扫描，并抽检分页、公式、图表、页眉页脚。任何错误或无合理覆盖依据的预警都应修复后重新生成。
...
全国赛默认交付 `PROJECT_ROOT/完整论文.docx`（纸质版源稿）、渲染出的 `完整论文-纸质版.pdf`、已删除前两页并通过检查的 `完整论文.pdf` 电子版，以及 `支撑材料.zip` 和其文件清单。用户显式要求 LaTeX 时同时交付 `完整论文-LaTeX/` 和 `.build.json`；只要一种格式时只交付指定格式。临时 Markdown、内部映射、检查日志和中间文件不作为交付物。
