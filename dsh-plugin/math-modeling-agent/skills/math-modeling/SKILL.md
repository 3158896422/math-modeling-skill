...
目标为 CUMCM 2026 时，必须使用用户提供的当届官方模板；内置模板只能作为技术基线，不能冒充官方模板。纸质版固定页面顺序为第 1 页承诺书、第 2 页编号专用页、第 3 页摘要专用页、第 4 页起正文，不要目录；摘要原则上不超过 1 页，正文不超过 30 页。渲染完整 PDF 后必须运行 `tools/docx/scripts/cumcm_submission.py`：纸质版核验页面顺序和正文边界，电子版删除前两页并保证首页为摘要、大小不超过 20 MB；支撑材料打包为一个 ZIP/RAR、不超过 20 MB 并生成文件清单。论文与支撑材料都要做通用敏感词加用户提供真实词条的匿名扫描，最后人工复核。
