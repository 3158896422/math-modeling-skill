...
const GATES = {
  M1: { phase: 'modeling', title: '建模终检', after: '进入编程或交付建模产物', checks: ['子问题覆盖', '模型合同', '信息时序', '合理基线/简化对照', '假设依据', '公式与符号', '单位与约束', '模型数量', '可实现性', '验证方案', '文献可追溯性'] },
  P1: { phase: 'programming', title: '最小可运行结果', after: '开始全量计算、正式出图和参数扫描', checks: ['从 PROJECT_ROOT 执行最小命令', '真实输入或等价小实例', '退出码', '关键结果', '公式/代码映射', '单位/范围/约束', '边界/守恒或训练评估隔离', '模型合同'] },
  P2: { phase: 'programming', title: '编程终检', after: '进入论文阶段或交付编程产物', checks: ['独立复现', '输入哈希', '随机种子', '关键数值', '边界与量纲', '分层验证', '情景/灵敏度', '多初值/多种子稳定性', '各子问题图表数量与语义', '复现清单', '文件完整性'] },
  W1: { phase: 'paper', title: '证据大纲', after: '开始长篇正文和排版', checks: ['模型合同', '基线/对照', '每个子问题结论有公式/结果表位置/图文件/验证层次/适用边界/代码输出/已核验文献支撑'] },
  W2: { phase: 'paper', title: '论文终检', after: '按用户要求交付论文并宣称完整完成', checks: ['当届规则', '主张—证据', '信息时序', '数值与单位', '基线和稳定性表述', '图表引用', '文献', '适用边界', '证据强度用词', '实际渲染', 'Word/LaTeX 一致性(双格式时)'] },
}
