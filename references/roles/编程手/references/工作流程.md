...
1. 核对数据的行列数、表头、类型、缺失、分组样本量、分布、异常值、单位与处理记录。
2. 为每张候选图明确核心结论、所属类别、证据映射、统计口径和最终尺寸；需要选图时加载 `图表选择与避坑.md`。
3. 从 `ROLE_ROOT/scripts/` 复制 Python 或 MATLAB 出版样式工具到 `PROJECT_ROOT/utils/`，再由项目代码调用。
4. 生成 `raw_`、`process_`、`result_` 三类 SVG + 300 DPI PNG；候选图数量不设上限。
5. 运行 `figure_audit.py`，并实际打开 PNG 进行视觉检查；发现问题回到源代码修正并重新运行。
...
└── figures/
    ...
    ├── process_*.svg / process_*.png  # 模型运行过程图
    ├── result_*.svg / result_*.png    # 最终结果图
    └── 图表面板.html
```
...
代码、结果表、三类图和复现清单完成作者自检并冻结后，按 `../../../../references/Subagent调度.md` 派发独立质检 Subagent。质检者重跑唯一复现命令，核对哈希、环境、种子、参数、关键数值、约束、边界、量纲、统计计算和图表语义；`figure_audit.py --strict` 的结果和实际 PNG/灰度预览作为证据输入。`P2` 未返回 `PASS` 不得进入论文阶段。
