...
第四步：质量检查
    ↓
    自审框架：论证逻辑 → 章节结构 → 表述质量 → 格式规范
    去AI味检查
```

**完整写作规范参见**：`./roles/论文手/references/工作流程.md` 和 `./roles/论文手/references/自审框架.md`
...
├─────────────────────────────────────────────────┤
│                                                 │
│  1. 摘要 ⭐ 最重要（context→gap→approach→       │
│                result→implication→boundary)     │
│  2. 问题重述 / 引言                             │
│     (field scale→bottleneck→prior→gap→present) │
│  3. 模型假设                                    │
│  4. 符号说明                                    │
│  5. 模型建立                                    │
│  6. 模型求解与结果（证据阶梯）                   │
│     system→validation→main result→comparison    │
│  7. 模型评价（讨论+结论）                        │
│     advance→evidence→relation→constraints       │
│  8. 相关工作（美赛需要）                         │
│  9. 参考文献                                   │
│  10. 附录                                       │
│                                                 │
...
**各章节详细架构模式参见**：`./roles/论文手/references/章节模板.md`

**美赛写作注意**：美赛论文使用英文撰写，摘要无需关键字行，需要独立 Related Work 章节。
详见 `./roles/论文手/references/英文化工作流.md` 和 `./roles/论文手/references/自审框架.md`。
...
```
context（背景）→ gap（缺口）→ approach（方法）
→ result（结果）→ implication（意义）→ boundary（边界）
```

| 要素 | 对应的问题 | 示例 |
|------|-----------|------|
| **context** | 研究背景是什么？ | 物流配送成本占电商总成本的30%以上 |
| **gap** | 现有方法有什么不足？ | 传统路径规划方法难以应对动态交通 |
| **approach** | 本文做了什么？ | 提出改进遗传算法，引入实时交通因子 |
| **result** | 具体结果？ | 配送距离缩短18.3%，成本降低22.5% |
| **implication** | 这意味着什么？ | 为大规模电商配送提供可行方案 |
| **boundary** | 在什么条件下成立？ | 城市配送场景，客户密度>10/km² |

**字数要求**：至少300字，不到1000字，**绝不超过一页纸**。
**关键字**：3到5个（国赛），美赛无需关键字。

**详细摘要模式参见**：`./roles/论文手/references/章节模板.md`
...
- **[📘 建模手](./roles/建模手/SKILL.md)** - 建模分析阶段子Skill（渐进式加载）
- **[💻 编程手](./roles/编程手/SKILL.md)** - 代码实现阶段子Skill（渐进式加载）
- **[📝 论文手](./roles/论文手/SKILL.md)** - 论文撰写阶段子Skill（渐进式加载）
